function results = fvar(y,nlag,x)
% PURPOSE: performs frequentist vector autogressive estimation
%---------------------------------------------------
% USAGE:  result = fvar(y,nlag,x) 
...
[nobs2 nx] = size(x);
 if (nobs2 ~= nobs)
 error('fvar: nobs in x-matrix not the same as y-matrix');
 end;
end;
