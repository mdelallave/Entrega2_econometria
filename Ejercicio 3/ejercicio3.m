...
% Contratamos un test sobre razón de verosimilitud para determinar el orden óptimo 
% del VAR.
sims = 1;
maxlag = 16;
minlag = 1;
...
% Calculando las predicciones del VAR
for i = 1:N
...
% En el test de Grenger obtenemos lo siguiente:
% -- En la regresión de BBVA, parece que todas las variables contribuyen a
% explicar la vol de BBVA menos el indice bancario FTS100
% -- En la regresión de Sabadell, parece que todas las variables, con sus
% retrasos, explican la vol de Sabadell (rechazamos todas las H0 a un 1%).
% -- En la regresión de Santander, Sabdell no explica a un 5% la vol de
% Santander. Mientras que el Indice Bancaria explica la volatilidad del
% SAN a partir del 2.4%.
% -- En la regresión VAR de IBEX-35, de nuevo todas las variables, con sus
% retrasos, parecen explicar la vol del IBEX-35 (rechazamos H0)
% -- En la regresión sobre el Indice Bancario lo mismo que para IBEX35 y
% SAbadell
...
Deltat = Deltat*100;
disp('_____________________________________________________');
disp('Connectedness Table');
disp('Cada elemento (i,j) nos dice en qué porcentaje el shock de la variable j explica la varianza del error de predicción de la variable i');
Deltat

% MDeltat_b=deltat_boot(err,yhat,100);
% Deltat=prctile(MDeltat_b,50,3);
% disp('Connectedness Table');
% disp('Cada elemento (i,j) nos dice en qué porcentaje el shock de la variable j explica la varianza del error de predicción de la variable i');
% prctile(MDeltat_b,50,3)
% %
% disp('Connectedness Table: percentil 97.5');
% prctile(MDeltat_b,97.5,3)
% disp('Connectedness Table: percentil 2.5');
% prctile(MDeltat_b,2.5,3)
%
%
disp('_____________________________________________________');
...
tot_i = sum(Deltat,2)-diag(Deltat)
tabla2 = table(tot_i)
disp('_____________________________________________________');
...
tot_j = sum(Deltat,1)-diag(Deltat)'
tabla3 = table(tot_j)
...
plot(G,'Layout','force','EdgeLabel',G.Edges.Weight,'LineWidth',LWidth,'ArrowSize',10,'markersize',20,'NodeCData',VNodeCData,'EdgeColor','k')
axis equal off
%plot(G,'Layout','layered','EdgeLabel',G.Edges.Weight,'LineWidth',LWidth,'ArrowSize',10,'markersize',20)
axis equal off
